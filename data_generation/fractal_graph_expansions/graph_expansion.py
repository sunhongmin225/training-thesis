...
def _compute_and_write_row_block(
  ...
  logging.info("%d interactions written in shard.",
               num_train_interactions)

  return (num_removed_rows, metadata, train_metadata)


def output_randomized_kronecker_to_pickle(
    left_matrix, right_matrix,
    train_indices_out_path, 
  ...
  """
  logging.info("Writing item sequences to pickle files %s.",
               train_indices_out_path)
  ...
  for i in xrange(left_matrix.shape[0]):
    ...
    (shard_num_removed_rows, shard_metadata, shard_train_metadata) = _compute_and_write_row_block(i, left_matrix, right_matrix, train_indices_out_path, train_metadata_out_path, remove_empty_rows)
  ...
  return metadata, train_metadata
