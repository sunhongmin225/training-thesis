...
import pytorch_gaussian_filter
import filters
...
def resize(image, output_shape, order=None, mode='reflect', cval=0, clip=True,
    ...
    print("msh::input_type: {}".format(input_type))
    print("msh::input_shape: {}, output_shape: {}".format(input_shape, output_shape))
    ...
    if anti_aliasing:
        if anti_aliasing_sigma is None:
            print("msh::anti_aliasing_sigma before: {}".format(anti_aliasing_sigma))
            anti_aliasing_sigma = np.maximum(0, (factors - 1) / 2)
            print("msh::anti_aliasing_sigma after: {}".format(anti_aliasing_sigma))
        else:
            anti_aliasing_sigma = \
        ...
        logging.info("msh::start ndi.gaussian_filter")
        print("msh::image.shape: {}, anti_aliasing_sigma: {}, cval: {}, ndi_mode: {}".format(image.shape, anti_aliasing_sigma, cval, ndi_mode))
        row, col = image.shape
        # movielens msh::image.shape: (138240, 2), anti_aliasing_sigma: [34559.5     0. ], cval: 0, ndi_mode: mirror
        # ours msh::anti_aliasing_sigma: [27764988.5        0. ], cval: 0, ndi_mode: mirror
        smoothing = pytorch_gaussian_filter.GaussianSmoothing(channels=1, kernel_size=3, sigma=(anti_aliasing_sigma[0] + 1, anti_aliasing_sigma[1] + 1), dim=2)
        # kernel_size = (501, 1) # msh::TODO figure out
        # blurrer = T.Compose([T.GaussianBlur(kernel_size=kernel_size, sigma=(anti_aliasing_sigma[1] + 1, anti_aliasing_sigma[0]))])
        print("msh::image before: {}".format(image))
        image = torch.from_numpy(image)
        image = torch.reshape(image, (1, row, col))
        image = torch.unsqueeze(image, 0)
        print("msh::image.shape before: {}".format(image.shape))
        image_pad = F.pad(image, (1, 1, 1, 1), mode='reflect')
        print("msh::image_pad.shape before: {}".format(image_pad.shape))
        # image = ndi.gaussian_filter(image, anti_aliasing_sigma, cval=cval, mode=ndi_mode)
        # image = filters.gaussian_filter(image, anti_aliasing_sigma, cval=cval, mode=ndi_mode)
        # image = blurrer(image)
        image = smoothing(image_pad)
        image = torch.squeeze(image)
        print("msh::image after smmothing: {}".format(image))
        print("msh::image.shape after smoothing: {}".format(image.shape))
        print("msh::image.type(): {}".format(image.type()))
        logging.info("msh::done ndi.gaussian_filter")
